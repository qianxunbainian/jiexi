...
if ($type !== '' && $url !== '') {
    // 统一顶层响应结构：code/msg/data
    $infoJson = getMusicInfo($type, $url);
    $info = json_decode($infoJson, true);
    if (!is_array($info)) {
        echo json_encode(['code' => 500, 'msg' => '解析失败', 'data' => []], 480);
        exit;
    }
    echo json_encode(['code' => 200, 'msg' => '解析成功', 'data' => $info], 480);
}else{
    echo json_encode(['code' => 404,'msg'=>'请补全参数','data'=>[]],480);
}
...
function getMusicInfo($type = 'json', $url = '')
{
    ...
    $info = array(
        ...
        'lyrics' => $lyrics,
        'core' => '抖音汽水音乐解析',
        'copyright' => '接口编写:OE源码网 接口编写:OE源码网 2025-4-20'
    );
