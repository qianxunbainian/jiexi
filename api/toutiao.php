...
$url = $_GET['url'] ?? '';
if (empty($url)) {
    echo json_encode(['code' => 201, 'msg' => 'url为空', 'data' => []], 480);
} else {
    $response = toutiao($url);
    if (empty($response)) {
        echo json_encode(['code' => 404, 'msg' => '获取失败', 'data' => []], 480);
    } else {
        // 统一适配 README.md 字段
        ...
        if ($code === 200) {
            $normalized = [
                ...
                'desc' => (string)($raw['description'] ?? ($raw['title'] ?? '')),
                'author' => [
                ...
                'music' => [
                    ...
                    'author' => (string)($raw['music']['author'] ?? ''),
                    'url' => $raw['music']['url'] ?? null,
                    'cover' => (string)($raw['music']['cover'] ?? ''),
                ],
                'video_id' => (string)($raw['itemId'] ?? ''),
                '_raw' => $raw,
            ];
        ...
        echo json_encode(['code' => $code, 'msg' => $msg, 'data' => []], 480);
    }
}
?>