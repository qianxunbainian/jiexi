...
class BilibiliParser
{
    ...
    {
        if (empty($url)) {
            return $this->jsonResponse(['code' => 400, 'msg' => '链接不能为空！', 'data' => []]);
        }
        ...
        if (!$bvid) {
            return $this->jsonResponse(['code' => 400, 'msg' => "视频链接好像不太对！", 'data' => []], 480);
        }
        ...
        if (($viewData['code'] ?? -1) == '0') {
            ...
            $responseData = [
                'code' => 200,
                'msg' => '解析成功！',
                // 统一适配 README.md 字段（原分P列表保留在 _raw）
                'data' => [
                    'type' => 'video',
                    'title' => (string)($viewData['data']['title'] ?? ''),
                    'desc' => (string)($viewData['data']['desc'] ?? ''),
                    'author' => [
                        'name' => (string)($viewData['data']['owner']['name'] ?? ''),
                        'id' => (string)($viewData['data']['owner']['mid'] ?? ''),
                        'avatar' => (string)($viewData['data']['owner']['face'] ?? ''),
                    ],
                    'cover' => (string)($viewData['data']['pic'] ?? ''),
                    'url' => isset($bilijson[0]['url']) && $bilijson[0]['url'] !== '' ? (string)$bilijson[0]['url'] : null,
                    // B站 pages.duration 单位是秒，这里按 README 统一输出毫秒
                    'duration' => isset($pages[0]['duration']) ? ((int)$pages[0]['duration'] * 1000) : null,
                    'video_backup' => [],
                    'images' => [],
                    'live_photo' => [],
                    'music' => [
            ...
            return $this->jsonResponse(['code' => 404, 'msg' => "解析失败！", 'data' => []]);
        }
    }
