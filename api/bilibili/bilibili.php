...
if (empty($urls)) {
    exit(json_encode(['code' => 400, 'msg' => '链接不能为空！', 'data' => []], 480));
}
$urls = cleanUrlParameters($urls);
$array = parse_url($urls);
if (empty($array)) {
    exit(json_encode(['code' => 400, 'msg' => "视频链接不正确", 'data' => []], 480));
    ...
    exit(json_encode(['code' => 400, 'msg' => "视频链接好像不太对！", 'data' => []], 480));
}
if (strpos($bvid, '/video/') === false) {
    exit(json_encode(['code' => 400, 'msg' => "好像不是视频链接", 'data' => []], 480));
}
...
if (empty($bvid)) {
    echo json_encode(['code' => 404, 'msg' => "获取id失败", 'data' => []], 480);
    exit;
}
...
if ($array['code'] == '0') {
    ...
    $mainUrl = $videos[0]['url'] ?? ($realVideoUrl ?? null);
    $mainDurationMs = isset($array['data']['pages'][0]['duration']) ? ((int)$array['data']['pages'][0]['duration'] * 1000) : null;
    $JSON = [
        'code' => 200,
        'msg' => '解析成功！',
        'data' => [
            ...
            'author' => [
                ...
                'id' => (string)($owner['mid'] ?? ''),
                'avatar' => (string)($owner['face'] ?? ''),
            ],
            'cover' => (string)$cover,
            'url' => is_string($mainUrl) && $mainUrl !== '' ? $mainUrl : null,
            'duration' => $mainDurationMs,
            'video_backup' => [],
            'images' => [],
            'live_photo' => [],
            'music' => [
