...
define('CONNECTION_TIMEOUT', 10); // 连接超时时间(秒)
...
function fetchVideoInfo($videoId, $headers)
{
    ...
    if (isset($responseData['code']) && $responseData['code'] == "100000") {
        $proxyBase = "https://wzapi.com/api/weibo?proxyurl=";
...
function outputSuccess($data)
{
    ...
    if (!is_array($data)) {
        echo json_encode(['code' => 500, 'msg' => '解析失败', 'data' => []], 480);
        exit;
    }
    ...
    if ($code === 200) {
        ...
        $videoBackup = [];
        if (isset($raw['quality_urls']) && is_array($raw['quality_urls'])) {
            foreach ($raw['quality_urls'] as $q => $u) {
                if (is_string($u) && $u !== '' && $u !== $videoUrl) {
                    $videoBackup[] = $u;
                }
            }
        }
        ...
        $normalized = [
            ...
            'video_id' => (string)($raw['video_id'] ?? ($raw['fid'] ?? ($raw['id'] ?? ''))),
            '_raw' => $raw,
        ];

        echo json_encode(['code' => 200, 'msg' => $msg ?: '解析成功', 'data' => $normalized], 480);
        exit;
    }

    echo json_encode(['code' => $code, 'msg' => $msg ?: '解析失败', 'data' => []], 480);
    exit;
}
...
function outputError($message, $code = 500)
{
    ...
    echo json_encode(['code' => (int)$code, 'msg' => (string)$message, 'data' => []], 480);
    exit;
}
