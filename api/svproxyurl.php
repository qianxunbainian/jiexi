...
function main()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        echo json_encode(['code' => 405, 'msg' => '仅支持GET或POST请求', 'data' => []], 480);
        exit;
    }
    ...
    if (empty($proxyurl)) {
        ...
        echo json_encode(['code' => 400, 'msg' => '参数proxyurl不能为空', 'data' => []], 480);
        exit;
    }
...
function weibo_proxy($proxyurl)
{
    ...
    if (empty($proxyurl) || !filter_var($proxyurl, FILTER_VALIDATE_URL)) {
        http_response_code(400);
        echo json_encode(['code' => 400, 'msg' => '无效的视频URL', 'data' => []], 480);
        return;
    }
    ...
    if ($ch === false) {
        ...
        echo json_encode(['code' => 500, 'msg' => 'cURL初始化失败', 'data' => []], 480);
        return;
    }
...
function douyin_proxy($proxyurl)
{
    if (empty($proxyurl)) {
        http_response_code(400);
        echo json_encode(['code' => 4001, 'msg' => '视频URL不能为空', 'data' => []], 480);
        return;
    }
    ...
    if (!$isValid) {
        http_response_code(400);
        echo json_encode(['code' => 4002, 'msg' => '视频URL格式无效', 'data' => []], 480);
        exit;
    }
