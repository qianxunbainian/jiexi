    ...
    private function formatNoteData($note)
    {
        ...
        if ($result['type'] == 'video') {
            ...
            if (!empty($streams)) {
                ...
                // 取第二高画质为备用链接 (如果存在)
                if (count($streams) > 1) {
                    $backupUrl = $streams[1]['masterUrl'] ?? null;
                    if (is_string($backupUrl) && $backupUrl !== '') {
                        $videoBackup[] = $backupUrl;
                    }
                }
            }
            ...
            $result['url'] = $videoUrl;
            // README.md 约定：video_backup 必须是数组（即便为空也返回 []）
            $result['video_backup'] = $videoBackup;
        }
