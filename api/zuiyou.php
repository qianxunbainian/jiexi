    ...
    if ($apiResponse === false) {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            $json = [
                ...
                'desc' => (string)($postData['content'] ?? ''),
                'author' => [
                    'name' => (string)($memberData['name'] ?? ''),
                    'id' => (string)($memberData['id'] ?? ''),
                    'avatar' => (string)($memberData['avatar_urls']['origin']['urls'][0] ?? ''),
                ],
                'cover' => (string)($imgsData['urls']['540_webp']['urls'][0] ?? ''),
                'url' => isset($videosData[$vid]['url']) ? (string)$videosData[$vid]['url'] : null,
                'duration' => null,
                'video_backup' => [],
                'images' => [],
                'live_photo' => [],
