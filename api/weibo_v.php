...
function weibo($url)
{
    $proxyurl = 'https://wzapi.com/api/weibo?proxyurl=';
...
$url = $_GET['url'];
if (empty($url)){
    $result = ['code' => 201, 'msg' => '链接不能为空！', 'data' => []];
} else {
    $info = weibo($url);
    // 检查 $info 是否为数组
    if (is_array($info) && $info['code'] == 200){
        $raw = is_array($info['data'] ?? null) ? $info['data'] : [];
        $normalized = [
            'type' => 'video',
            'title' => (string)($raw['title'] ?? ''),
            'desc' => (string)($raw['title'] ?? ''),
            'author' => [
        ...
        $result = ['code' => 200, 'msg' => (string)($info['msg'] ?? '解析成功'), 'data' => $normalized];
    } else{
        $result = ['code' => 404, 'msg' => '解析失败！', 'data' => []];
    }
}
