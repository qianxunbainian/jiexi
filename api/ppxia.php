    ...
    try {
        ...
        if (is_array($arr) && isset($arr['data']['cell_comments'][1]['comment_info']['item'])) {
            ...
            $result = [
                ...
                'data' => [
                    // 统一适配 README.md 字段
                    'type' => !empty($imgurl) ? 'image' : 'video',
                    'title' => $arr['data']['cell_comments'][1]['comment_info']['item']['content'] ?? '',
                    'desc' => $arr['data']['cell_comments'][1]['comment_info']['item']['content'] ?? '',
                    'author' => [
                        'name' => (string)($arr['data']['cell_comments'][1]['comment_info']['item']['author']['name'] ?? ''),
                        'id' => (string)($arr['data']['cell_comments'][1]['comment_info']['item']['author']['id'] ?? ''),
                        'avatar' => (string)($arr['data']['cell_comments'][1]['comment_info']['item']['author']['avatar']['download_list'][0]['url'] ?? ''),
                    ],
                    'cover' => (string)($arr['data']['cell_comments'][1]['comment_info']['item']['cover']['url_list'][0]['url'] ?? ''),
                    'url' => empty($imgurl) ? $video_url : null,
                    'duration' => null,
                    'video_backup' => [],
                    'images' => $imgurl,
                    'live_photo' => [],
