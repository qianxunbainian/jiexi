...
function short_videos($url) {
    ...
    if (!filter_var($url, FILTER_VALIDATE_URL)) {
        outputJson(['error' => '请提供有效的URL参数'], 400);
    }

    // 单接口转发到本地 sv1，域名和部署路径自适应
    $apiConfig = [
        'base_url' => getCurrentBaseUrl() . getProjectBasePath() . '/short_videos/sv1.php',
        'timeout' => 30, // 超时时间(秒)
...
// 验证参数是否存在
if (empty($url)) {
    outputJson(['error' => '必须提供url参数'], 400);
}
